...
The app builds and runs fully **offline** without any of this. When no Supabase
credentials are present the online layer automatically falls back to a built-in
**local simulation backend** (`src/online/mockBackend.ts`): friends, friend
requests, the Friend-ID flow, leaderboards, and duels against a simulated
opponent all work end-to-end with **zero external servers**. This means the
game is never blocked by a missing/invalid setup — there is no hard dependency
on this file or on Supabase to launch and test the multiplayer screens.

> **Local/dev mode (default):** do nothing — just `npm run dev` (or build & run
> in Xcode). The ⚔️ online button, Friends, and Leaderboard screens are live
> against the simulation. Set `VITE_OFFLINE_MODE=1` to force it even when real
> creds exist.
>
> **Production mode:** add the two env vars below to switch to the real,
> server-authoritative Supabase backend.
