...
//
// LOCAL MODE: when no Supabase backend is configured (isLocalMode()), every
// operation is transparently delegated to the in-memory simulation backend
// (mockBackend.ts) so the entire online layer is testable without any server.
...
export async function ensureSession(): Promise<string> {
  if (isLocalMode()) return mock.ensureSession();
...
export async function ensureProfile(name?: string, icon?: string, color?: string, code?: string): Promise<Profile> {
  if (isLocalMode()) return mock.ensureProfile(name, icon, color, code);
  const sb = requireClient();
...
export async function getProfile(id: string): Promise<Profile | null> {
  if (isLocalMode()) return mock.getProfile(id);
  const sb = requireClient();
...
export async function touchLastSeen(): Promise<void> {
  if (isLocalMode()) return mock.touchLastSeen();
...
export async function syncServerClock(): Promise<void> {
  if (isLocalMode()) return mock.syncServerClock();
  const sb = getClient(); if (!sb) return;
  const t0 = Date.now();
...
export async function sendFriendRequest(code: string): Promise<FriendEdge> {
  if (isLocalMode()) return mock.sendFriendRequest(code);
  const sb = requireClient();
...
export async function respondFriend(id: string, accept: boolean): Promise<void> {
  if (isLocalMode()) return mock.respondFriend(id, accept);
  const sb = requireClient();
...
export async function listFriendEdges(me: string): Promise<FriendEdge[]> {
  if (isLocalMode()) return mock.listFriendEdges(me);
  const sb = requireClient();
...
export async function listFriends(me: string): Promise<FriendView[]> {
  if (isLocalMode()) return mock.listFriends(me);
  const sb = requireClient();
...
export async function listIncomingRequests(me: string): Promise<RequestView[]> {
  if (isLocalMode()) return mock.listIncomingRequests(me) as Promise<RequestView[]>;
  const sb = requireClient();
...
export const leaderboardGlobal = (limit = 100): Promise<LeaderRow[]> =>
  isLocalMode() ? mock.leaderboardGlobal(limit) : lb("leaderboard_global", { p_limit: limit });
export const leaderboardWeekly = (limit = 100): Promise<LeaderRow[]> =>
  isLocalMode() ? mock.leaderboardWeekly(limit) : lb("leaderboard_weekly", { p_limit: limit });
export const leaderboardFriends = (): Promise<LeaderRow[]> =>
  isLocalMode() ? mock.leaderboardFriends() : lb("leaderboard_friends");
...
export async function quickMatch(cfg: MatchConfig): Promise<string> {
  if (isLocalMode()) return mock.quickMatch(cfg);
  const sb = requireClient();
...
export async function createInvite(friend: string, cfg: MatchConfig): Promise<string> {
  if (isLocalMode()) return mock.createInvite(friend, cfg);
  const sb = requireClient();
...
export async function acceptInvite(matchId: string): Promise<void> {
  if (isLocalMode()) return mock.acceptInvite(matchId);
  const sb = requireClient();
  const { error } = await sb.rpc("accept_invite", { p_match: matchId });
  if (error) throw error;
}
export async function cancelMatch(matchId: string): Promise<void> {
  if (isLocalMode()) return mock.cancelMatch(matchId);
  const sb = getClient(); if (!sb) return;
...
export async function submitProgress(matchId: string, correct: number, answered: number,
  ...
  if (isLocalMode()) return mock.submitProgress(matchId, correct, answered, timeMs, finished);
  const sb = requireClient();
...
export async function finishMatch(matchId: string): Promise<void> {
  if (isLocalMode()) return mock.finishMatch(matchId);
  const sb = getClient(); if (!sb) return;
  await sb.rpc("finish_match", { p_match: matchId });
}
export async function getMatch(id: string): Promise<Match | null> {
  if (isLocalMode()) return mock.getMatch(id);
  const sb = requireClient();
...
export async function getMatchPlayers(id: string): Promise<MatchPlayer[]> {
  if (isLocalMode()) return mock.getMatchPlayers(id);
  const sb = requireClient();
...
export function subscribeMatch(id: string, onChange: () => void): Unsub {
  if (isLocalMode()) return mock.subscribeMatch(id, onChange);
  const sb = requireClient();
...
export function subscribeInvites(me: string, onInvite: () => void): Unsub {
  if (isLocalMode()) return mock.subscribeInvites(me, onInvite);
  const sb = requireClient();
...
export function subscribeFriends(_me: string, onChange: () => void): Unsub {
  if (isLocalMode()) return mock.subscribeFriends(_me, onChange);
  const sb = requireClient();
